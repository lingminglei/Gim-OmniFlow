...
// --- 自定义 Less 变量 (用于取代外部依赖) ---
@PrimaryColor: #409EFF;       // 主题色，El-Button Primary
@PrimaryHover: #ecf5ff;       // 主题色的浅色 Hover 背景
@RegularText: #606266;        // 默认文本色
@BorderLighter: #ebeef5;      // 边框色
@DangerColor: #F56C6C;        // 危险操作色

// --- Mixins 模拟 (Less Function) ---
.setEllipsis(@lines: 1) when (@lines = 1) {
    overflow: hidden;
    white-space: nowrap;
...
.unzip-list {
    ...
    // 覆盖 Element-UI 分割线样式
    /deep/ .el-divider {
        margin: 4px 0; // 调整分割线垂直间距，略大于 2px
    }
}
...
.right-menu-list {
    ...
    .unzip-item {
        ...
        line-height: 36px;
        cursor: pointer;
        transition: background 0.2s, color 0.2s; // 增加过渡效果
    ...
    &.add {
        .right-menu-item {
            ...
            i {
                margin-right: 8px;
                font-size: 18px;
                width: 20px; // 确保图标占位与图片一致
                text-align: center;
            }
        }
    }

    // --- 解压缩子菜单项样式 (.unzip-menu-item) ---
    .unzip-menu-item {
        position: relative;
        display: flex;
        ...
        .el-icon-arrow-right {
             margin-right: 0; // 箭头图标不需要右侧 margin
        }
        ...
        .unzip-list {
            ...
            display: none;
            list-style: none;
            min-width: 180px; // 确保宽度足够显示长文件名
            
            .unzip-item {
                .setEllipsis(1); // 应用单行省略号
                max-width: 200px;
            }
        }
    }
