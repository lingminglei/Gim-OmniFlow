		...
		<div class="code-preview-wrapper" v-show="visible" @keydown.s.ctrl.prevent="handleModifyFileContent">
			...
			<div class="tip-wrapper" v-if="visible">
				...
				<div class="tool-wrapper">
					...
					<i class="item el-icon-close" title="关闭预览" @click="closeCodePreview"></i>
				</div>
			</div>
			<!-- 代码编辑区域 -->
			<div class="code-editor-wrapper">
				<div class="operate-wrapper">
					<i class="save-icon iconfont icon-baocun" title="保存（ctrl+s）"
						v-show="isModify && !codeMirrorOptions.readOnly" @click="handleModifyFileContent"></i>
					<el-form class="editor-set-form" :model="codeMirrorOptions" :inline="true" size="small"
						label-position="right" label-suffix=":">
						<el-form-item label-width="0px" class="line-wrapper">
							<el-checkbox v-model="codeMirrorOptions.lineWrapping"
								@change="handleChangeCodeMirrorOption">自动换行</el-checkbox>
						</el-form-item>
						<el-form-item label-width="0px" class="font-size">
							<el-select v-model="codeMirrorCustomOptions.fontSize" filterable>
								<el-option v-for="(item, index) in fontSizeList" :key="index" :value="item"
									:label="`${item} px`"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label-width="80px" label="代码语言" class="lanaguage">
							<el-select v-model="codeMirrorOptions.mode" filterable
								@change="handleChangeCodeMirrorOption">
								<el-option v-for="(value, key, index) in fileSuffixCodeModeMap" :key="index"
									:value="value[1].mime" :label="value[1].language"></el-option>
							</el-select>
						</el-form-item>
						<el-form-item label="主题" label-width="56px" class="theme">
							<el-select v-model="codeMirrorOptions.theme" filterable
								@change="handleChangeCodeMirrorOption">
								<el-option value="default" label="default"></el-option>
								<el-option v-for="(item, index) in codeMirrorThemeList" :key="index" :label="item"
									:value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-form>
				</div>
				<codemirror class="code-editor" ref="codemirrorRef" v-model="codeMirrorText"
					:options="codeMirrorOptions" v-loading="codeMirrorLoading" v-if="isShow"
...
export default {
	...
	methods: {
		...
		getCodeText() {
			...
			if (this.fileInfo.sourceType === undefined || this.fileInfo.sourceType === ''
				|| this.fileInfo.sourceType === null
			) {
