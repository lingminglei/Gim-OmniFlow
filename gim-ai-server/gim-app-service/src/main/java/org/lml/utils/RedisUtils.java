...
public final class RedisUtils {
    ...
    // 定义 Key 的前缀，用于任务完成标记
    private static final String TASK_COMPLETED_PREFIX = "TASK_COMPLETED:";

    // 设置标记的过期时间（例如 7 天），足够覆盖业务周期
    private static final long TASK_COMPLETED_EXPIRATION_DAYS = 7;
    ...
    /**
     * 修改redis-任务执行状态,需要前置状态校验
     */
    public boolean compareAndSetStatus(String key, String expectStatusList, String newStatus) {
        // 优化后的脚本：增加精确匹配逻辑
        String script =
                "local raw = redis.call('HGET', KEYS[1], 'status')\n" +
                        "local curStatus = \"NONE\"\n" +
                        "if raw then\n" +
                        "    -- 同时去除引号和可能存在的转义反斜杠\n" +
                        "    curStatus = tostring(raw):gsub('\"', ''):gsub('\\\\', ''):gsub('%s+', '')\n" +
                        "end\n" +
                        "\n" +
                        "-- 2. 强力清洗传入的 允许列表 (ARGV[1])\n" +
                        "-- 无论 Java 传过来的是 \"INIT,SUCCESS\" 还是 \"\\\"INIT,SUCCESS\\\"\" 全部洗白\n" +
                        "local cleanAllowed = tostring(ARGV[1]):gsub('\"', ''):gsub('\\\\', ''):gsub('%s+', '')\n" +
                        "\n" +
                        "-- 3. 构造匹配边界\n" +
                        "local allowedStr = ',' .. cleanAllowed .. ','\n" +
                        "local targetStr = ',' .. curStatus .. ','\n" +
                        "\n" +
                        "-- 4. 匹配判断\n" +
                        "if string.find(allowedStr, targetStr, 1, true) then\n" +
                        "    -- 注意：写入时 ARGV[2] 建议由外部控制是否带引号，或者这里统一处理\n" +
                        "    -- 这里我们维持原样写入，但通常建议存入不带引号的字符串\n" +
                        "    redis.call('HSET', KEYS[1], 'status', ARGV[2], 'updateTime', ARGV[3])\n" +
                        "    return \"1\"\n" +
                        "else\n" +
                        "    -- 失败返回详细信息，方便你确认\n" +
                        "    return \"FAIL_DETAIL: ALLOWED=\" .. allowedStr .. \" VS TARGET=\" .. targetStr\n" +
                        "end";

        // 建议：将 DefaultRedisScript 抽离为常量以提高性能
        DefaultRedisScript<Long> redisScript = new DefaultRedisScript<>(script, Long.class);

        log.info("expectStatusList={},newStatus={},data={},",expectStatusList,newStatus,Collections.singletonList(key));

        Object result = redisTemplate.execute(
                redisScript,
                Collections.singletonList(key),
                expectStatusList, // 传入如 "INIT,SUBMITTED"
                newStatus,        // 目标状态
                DateUtil.now()    // 更新时间
        );
        boolean isSuccess = false;
        if (result instanceof Number) {
            isSuccess = ((Number) result).intValue() == 1;
