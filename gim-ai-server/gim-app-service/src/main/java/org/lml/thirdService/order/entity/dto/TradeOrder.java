...
import org.lml.thirdService.order.entity.req.*;
...
public class TradeOrder extends BaseEntity {
    ...
    public Boolean isTimeout() {
        ...
        return (orderState == TradeOrderState.CLOSED && closeType == TradeOrderEvent.TIME_OUT.name())
                || (orderState == TradeOrderState.CONFIRM && this.getCreateTime().compareTo(DateUtils.addMinutes(new Date(), -TradeOrder.DEFAULT_TIME_OUT_MINUTES)) < 0);
    }
    ...
    public Date getPayExpireTime() {
        return DateUtils.addMinutes(this.getCreateTime(), TradeOrder.DEFAULT_TIME_OUT_MINUTES);
    }
    ...
    public TradeOrder confirm() {
        this.setOrderConfirmedTime(new Date());
        TradeOrderState orderState = OrderStateMachine.INSTANCE.transition(this.getOrderState(), TradeOrderEvent.CONFIRM);
