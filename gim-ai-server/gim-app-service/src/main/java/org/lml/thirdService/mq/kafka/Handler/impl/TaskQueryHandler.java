...
//@Component
//@Slf4j
//public class TaskQueryHandler implements MessageHandler<String> {
//
//    @Resource
...
//
//            log.info("任务生成成功!,文件地址为：{}",fileUrl);
//
//            //文件转存到 OSS 服务，获取文件公网预览地址
...
//
//        /**
//         * TODO: 这里留用，后续处理为通过 香蕉调用
//         */
////        if(taskState.equals("0")){
////            log.info("处理状态：【{}】，任务正在排队处理中.",taskState);
////        } else if(taskState.equals("1")){
////            log.info("处理状态：【{}】,任务正在生成中.",taskState);
////        } else if(taskState.equals("3")){
////            log.info("处理状态：【{}】,任务生成失败！",taskState);
////            redisUtils.hset(key, "status", TaskState.FAIL.getCode());
////            redisUtils.hset(key, "updateTime", DateUtil.now());
////        } else {
////            log.info("任务生成成功!,文件地址为：{}",taskState);
////            redisUtils.hset(key, "status", TaskState.SUCCESS.getCode());
...
//
//    @Override
//    public String getSupportedType() { return MqBusinessEnum.QUERY_IMAGE_RESULT.getCode(); }
//}
